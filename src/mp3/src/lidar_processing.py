...
class LidarProcessing:
    def __init__(
        ...
        # Initialize ROS 2 publishers and subscribers if node is provided
        if self.node is not None:
            self.birdsEyeViewPub = self.node.create_publisher(Image, "/mp3/BirdsEye", 1)
            self.pointCloudSub = self.node.create_subscription(PointCloud2, "/velodyne/points", self.__pointCloudHandler, 10)
        else:
            self.birdsEyeViewPub = None
            self.pointCloudSub = None
    ...
    def __pointCloudHandler(self, data):
        ...
        """
        gen = point_cloud2.read_points(cloud=data, field_names=('x', 'y', 'z'))
